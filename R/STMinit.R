...
stm.init <- function(documents, settings,refBeta = NULL) {
  ...
  if(mode=="Spectral" | mode=="SpectralRP") {
    ...
    if(mode=="Spectral") {
      ...
      Q <- stm:::gram.rp(mat, s=settings$init$s, p=settings$init$p,
    ...
    if(K!=0) {
      ...
      anchor <- stm:::fastAnchor(Q, K=K, verbose=verbose)
    } else {
      if(verbose) cat("\t Finding anchor words...\n \t")
      anchor <- stm:::tsneAnchor(Q, verbose=verbose,
    ...
    beta <- stm:::recoverL2(Q, anchor, wprob, verbose=verbose, recoverEG=settings$init$recoverEG)$A
    if(!is.null(keep)) {
  ...
  if(!settings$kappa$LDAbeta) {
    model$kappa <- kappa.init(documents, K, V, A, interactions=settings$kappa$interactions,refBeta)
  }
...
###
kappa.init <- function(documents, K, V, A, interactions,refBeta = NULL) {
  kappa.out <- list()
