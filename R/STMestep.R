...
estep <- function(documents, beta.index, update.mu, #null allows for intercept only model
                       beta, refBeta=NULL,lambda.old, mu, sigma,
                       verbose) {
  ...
  for(i in 1:N) {
    ...
    if(!is.null(refBeta)){
      beta.ss[[aspect]][-(1:dim(refBeta)[1]),words] <- doc.results$phis[-(1:dim(refBeta)[1]),] + beta.ss[[aspect]][-(1:dim(refBeta)[1]),words]
      beta.ss[[aspect]][1:dim(refBeta)[1],words] <- refBeta[,words]
    }
