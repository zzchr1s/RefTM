...
mnreg <- function(beta.ss,settings,refBeta = NULL) {
  #Parse Arguments
  ...
  if(A!=1) { #Topic-Aspect Models
    if(!contrast) {
      ...
      vecj <- c(vecj,rep(((K-K0)+1):((K-K0)+A), each=(K-K0)))
      if(interact) {
        veci <- c(veci, 1:nrow(counts))
        vecj <- c(vecj, ((K-K0)+A+1):((K-K0)+A+nrow(counts)))
      }
      ...
      if(interact) {
        ...
        eye <- Matrix::Matrix(diag((K-K0)*A))
        eye <- eye[,((K-K0)+1):ncol(eye)] #take the identity matrix but drop out the first aspect
  ...
  beta <- split(beta, rep(1:A, each=(K-K0)))
  beta <- lapply(beta, matrix, nrow=(K-K0))
  
  if(!is.null(refBeta)){
    for(i in 1:length(beta)){
        beta[[i]] = rbind(refBeta,beta[[i]])
    }
  }
