...
##RefTM-LDA workflow
  if(workflow == "LDA"){
    ...
    if(!is.null(k2)){
      ...
      model_sc <- RefTM.LDA(sc_data, k = k1+5, method = "VEM", k0 = k1, bulk_beta = model_ref@beta, control = list(estimate.alpha = F))
    }else{
      print("Select the best model.")
      k <- k1 + c(5,10,15,20,25)
      models <- list()
      perplex <- c()
      for (i in 1:5) {
      ...
      model_sc <- models[(which(log.lik$second_derivative == max(log.lik$second_derivative)) - k1)/5]
    }
  }else
...
##RefTM-STM workflow
  if(workflow == "STM"){
    ...
    if(!is.null(k2)){
      sprintf("Number of topics is fixed to be %s.", k1+5)
      model_sc <- RefTM.STM(getCorpus(sc_data), as.character(1:dim(sc_data)[2]), K = k1+5, prevalence = ~covariate, refBeta = exp(model_ref$beta$logbeta[[1]]))
    }else{
      print("Select the best model.")
      perp = function(model){
      ...
      perplex <- c()
      for (i in 1:5) {
        models[i] <- RefTM.STM(getCorpus(sc_data), as.character(1:dim(sc_data)[2]), K = k[i], prevalence = ~covariate, refBeta = exp(model_ref$beta$logbeta[[1]]))
