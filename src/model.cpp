...
int model::init(int *i, int *j, int *v, int total, double *delta, int *Z, double *Phi, int init, int K0, double *refBeta) {
    int m, n, w, k;
...
void model::estimate(double *refBeta) {
  if (verbose > 0) Rprintf("Sampling %d iterations!\n", niters);
	...
	if ((save > 0) && (liter % save == 0)) {
	  ...
	  compute_theta();
	  if (estimate_phi == 1) compute_phi(refBeta);
...
void model::compute_phi(double* refBeta) {
  if (seeded == 1) {
    for (int k = 0; k < K; k++) {
      for (int w = 0; w < V; w++) {
