...
void lda_mle(lda_model* model, lda_suffstats* ss, int estimate_alpha, int verbose, int NTOPICS_bulk)
{
    int k; int w;

    for (k = NTOPICS_bulk; k < model->num_topics; k++)
    {
...
lda_model* new_lda_model(int num_terms, int num_topics,double *refBeta, int num_topics_bulk)
{
    int i,j;
    lda_model* model;
