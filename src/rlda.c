...
static lda_model* R2lda_model(SEXP init_model,double *refBeta, int num_topics_bulk)
{
    ...
    model = new_lda_model(num_terms, num_topics,refBeta, num_topics_bulk);
    model->alpha = *REAL(GET_SLOT(init_model, install("alpha")));
    for (i = 0;  i < num_topics_bulk; i++)
      for (j = 0; j < num_terms; j++)
	model->log_prob_w[i][j] = refBeta[i + num_topics_bulk * j];

    for (i = num_topics_bulk;  i < num_topics; i++)
      for (j = 0; j < num_terms; j++)
	  ...
	  SEXP control, SEXP k, SEXP prefix, SEXP init_model, SEXP refBeta, SEXP k0)
{
  ...
  NTOPICS_bulk = *INTEGER(AS_INTEGER(k0));
  ...
  if (strcmp(start, "seeded")==0)
    {
      model = new_lda_model(corpus->num_terms, NTOPICS,REAL(refBeta),NTOPICS_bulk);
      ss = new_lda_suffstats(model);
      corpus_initialize_ss(ss, model, corpus);
      lda_mle(model, ss, 0, LAG > 0, NTOPICS_bulk);
    ...
    {
      model = R2lda_model(init_model,REAL(refBeta),NTOPICS_bulk);
      ss = new_lda_suffstats(model);
    }
  ...
  while (((converged < 0) || (converged > EM_CONVERGED) || (iter <= 2)) && (iter < EM_MAX_ITER))
    {
      ...
      lda_mle(model, ss, ESTIMATE_ALPHA, verbose,NTOPICS_bulk);
