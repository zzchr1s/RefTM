...
model* lda(int *i, int *j, int *v, int total, 
	  ...
	  int K, int K0, int M, int V, double alpha, double *beta,
	  string dir, double *init_phi, int *z, double *refBeta) 
{
  model* lda = new model();
  lda->niters = niters;
  lda->verbose = verbose;
  ...
  lda->alpha = alpha;
  lda->dir = dir;
  lda->init(i, j, v, total, beta, z, init_phi, model_status, K0, refBeta);
  lda->estimate(refBeta);
  lda->inference();
  return(lda);
}
...
SEXP rGibbslda(SEXP i, SEXP j, SEXP v, SEXP nrow, SEXP ncol,
	       ...
	       SEXP k, SEXP k0, SEXP prefix, SEXP phi, SEXP z, SEXP refBeta) 
{
  SEXP ans, cls;
